# これは「実発注せず、ベストBid/Askで疑似約定させる Paper ExchangeGateway」を提供するファイルです。
...
class PaperExchange(ExchangeGateway):
    ...
    async def place_order(self, req: OrderRequest) -> Order:
        ...
        if req.type.lower() == "market":
            ...
            return Order(order_id=po.order_id, client_id=po.client_id, status="filled", filled_qty=req.qty, avg_fill_price=price)
    ...
    async def _try_fill_limits(self, symbol: str) -> None:
        ...
        """
        async with self._lock:
            candidates = [po for po in self._orders.values() if po.req.symbol == symbol and po.status == "new" and po.req.type.lower() == "limit"]
    ...
    async def _apply_fill_effects(self, *, symbol: str, side: str, qty: float, price: float) -> None:
        ...
        async with self._lock:
            ...
            if new_size > 0:
                pos.entry_price = (pos.entry_price * pos.size + float(qty) * float(price)) / new_size if pos.size > 0 else float(price)
            pos.size = new_size
